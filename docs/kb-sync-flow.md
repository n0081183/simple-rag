...
|---------|---------|--------------|
| Request rate | **0.35 req/s** (UI slider on KB page) | `rate_limit_rps` in sync API; pod env `RATE_LIMIT` |
| Topic download workers | **1** (sequential) | `topic_workers` in sync API; `CORTEX_SYNC_TOPIC_WORKERS` |
